            ...
            <div className={cx('reviews-listing-item')}>
                <div className={cx('reviews-listing-content')}>
                    <div className={cx('reviews-rating')}>
                        {
                            [...Array(item.star || 0)].map((star, index) => {
                                return <img key={index} className={cx('star')} src="https://www.coolmate.me/images/star.svg?2a5188496861d26e5547c524320ec875" />
                            })
                        }
                    ...
                    <div className={cx('reviews-author')}>
                        <div className={cx('reviews-author-name')}> {item.user?.fullName}  </div>
                    ...
                    <div className={cx('reviews-listing-description')}>
                        ...
                        <div className={cx('reviews-listing-gallery')}>
                            {item.imagesRv?.map((itemImg, index) => {
                                return <a key={index} className={cx('reviews-listing-image')} >
                                    <img src={itemImg} />
                                </a>
                            })} </div>
                        {item.isResponsed &&
                        ...
                        {
                            item.reviewDate &&
                            <span className={cx('reviews-listing-date')}> {convertDate(item.reviewDate)} </span>
                        }
                    </div>
